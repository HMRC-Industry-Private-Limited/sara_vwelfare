...
publicWidget.registry.PaymentPostProcessing = publicWidget.Widget.extend({
            ...
            if (data.success === true) {
                ...
                switch (data.error) {
                    case "tx_process_retry":
                        break;
                    case "no_tx_found":
                        self._renderTemplate("payment.no_tx_found", {});
                        break;
                    default: // if an exception is raised
                        self._renderTemplate("payment.exception", { exception_msg: data.error });
                        break;
                }
            }
            self._updateTimeout();
    ...
    processPolledData: function (display_values_list) {
        ...
        */
        ...
        this._renderTemplate("payment.display_tx_list", render_values);
    },
